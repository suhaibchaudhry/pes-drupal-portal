...
<?php endif;?>
  <div class="<?php print $class; ?>" <?php print $data; ?>>
    <?php
	//dpm($layer);
        if(property_exists($layer, 'link') && strpos(strtolower($layer->link['value']), 'youtube.com/embed') !== FALSE) {
		//if(($_SERVER['HTTPS'] == 'on')) {
			$link = str_replace('http', 'https', $link);
		//}
                $layer->link['rel'] = 'lightframe[]';
        }
    ?>

    <?php if ($layer->type == 'text'): ?>
        <?php if (isset($link)):?>
            <a <?php if(isset($layer->link['rel'])) { print 'rel="'.$layer->link['rel'].'"'; } ?>  href="<?php print $link;?>"<?php if (isset($layer->link["target"]) && !empty($layer->link["target"])) print " target='{$layer->link["target"]}'";?>><?php print $layer->title;?></a>
        <?php else:?>
            <?php print $layer->title;?>
        <?php endif; ?>
    <?php elseif ($layer->type == 'image'): ?>
        <?php if (isset($link)):?>
            <a <?php if(isset($layer->link['rel'])) { print 'rel="'.$layer->link['rel'].'"'; } ?> href="<?php print $link;?>"<?php if (isset($layer->link["target"]) && !empty($layer->link["target"])) print " target='{$layer->link["target"]}'";?>>
